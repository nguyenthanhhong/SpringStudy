...
import com.nth.DynamicSpringBatchSchedule.model.SettingJobSchedule;
import com.nth.DynamicSpringBatchSchedule.repository.SettingJobScheduleRepo;
...
public class SchedulerConfig implements SchedulingConfigurer {
    ...
    private String[] getCurrentJobCondition(int jobId) {
        Optional<SettingJobSchedule> job = setting.findById(jobId);
        return (job.isPresent() && job.get().getParameter() != null) ? job.get().getParameter().split(",") : new String[0];
    }

    private String getCurrentJobCron(int jobId) {
        Optional<SettingJobSchedule> job = setting.findById(jobId);
        return (job.isPresent() && job.get().getCronExpression() != null) ? job.get().getCronExpression() : CRON_END_OF_THE_YEAR;
    }
        ...
        for (SettingJobSchedule job : addJobs) {
            ...
            if (newJob != null) {
                ...
                ScheduledFuture future = scheduledTaskRegistrar.getScheduler().schedule(
                        () -> newJob.run(job.getJobId(), getCurrentJobCondition(job.getJobId())),
                        t -> {
                            CronTrigger crontrigger = new CronTrigger(getCurrentJobCron(job.getJobId()));
